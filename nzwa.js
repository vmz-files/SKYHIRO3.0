	...
	nzwa.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '38988313755@s.whatsapp.net'
           ...
           break
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Pedido recebido, removi esta pessoa : @${mentioned[0].split('@')[0]}`, mentioned, true)
						nzwa.groupRemove(from, mentioned)
					nzwa.sendMessage(mentioned, 'Adeus ðŸ˜‚', text)
					}
					break
				case 'exe':
	              nzwa.updatePresence(from, Presence.composing) 
	          ...
	          if (!isRegister) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`teksnya mana? contoh ${prefix}logowolf Screamo|Lords`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
				...
				case 'logowolf2':
					...
					var gbl1 = gh.split("|")[0];
					var gbl2 = gh.split("|")[1];
					if (args.length < 1) return reply('Cadê o texto, hum\nExemplo: ${prefix}phlogo |Screamo|Canss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
            ...
            break
				        ...
				        uptime = process.uptime()
                                ...
                                case 'jsholat':
                                        loc = body.slice(8)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (args.length < 1) return reply('Onde fica a área ?')
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                ...
                                case 'coffee':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}coffee Lorddscreamo')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'qowheart':
                                        var gh = body.slice(9)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde está o texto hum\nExemplo: ${prefix}qowheart ScreamoCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'undergocean':
                                        var gh = body.slice(12)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}undergocean Screamos')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'woodenboards':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}woodenboards Screamonss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'wolfmetal':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}wolfmetal Screamos')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/photooxy?theme=wolf_metal&text=${teks1}&apikey=BotWeA`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
                                        nzwa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui está a foto...'})
                                        await limitAdd(sender)
                                        break
                                case 'metalictglow':
                                        var gh = body.slice(14)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde está o texto hum\nExemplo: ${prefix}metalictglow LordeScreamo')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
				...
				case 'ttp':
					...
					break
                                case 'clearall':
					if (!isOwner) return reply('❌voce nao tem permissao❌')
					anu = await nzwa.chats.all()
				...
				case 'clone':
					...
					break
                                case 'pubglogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}pubglogo Lordescreamo|Canss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply('onde está o texto hum\nContoh: ${prefix}harrypotter ScreamoCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'bal':
                                        ...
                                        break
                                case 'buylimit':
                                        if (args.length < 1) return reply('sem limite \n\nCara cek uang: ${prefix}bal')
                                        if (!isRegister) return reply(mess.only.daftarB)
